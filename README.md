...
Live mic capture + Hinglish demo simulator chips. The card below shows the last extracted event, straight from the Demand Capture Agent: raw transcript, matched canonical product, and stockout status.

![Demand Capture Terminal](docs/screenshots/demand_capture_terminal.png)

</td>
<td width="50%">

**AI Procurement Advisor**
Recommendations generated by the Procurement Agent, each with a priority badge (HIGH/MEDIUM/LOW), a plain-language reason, and Accept & Order / Dismiss actions the shopkeeper can act on directly.

![Procurement Advisor](docs/screenshots/procurement_advisor.png)

</td>
</tr>
</table>

The dashboard has two more tabs not pictured above — **Demand** (Pandas-aggregated demand scores per product) and **Insights** (live weather, festival countdown, market trend indexes) — powered by the Demand Intelligence and Business Intelligence agents respectively.
...
| Component | URL |
|---|---|
| **Backend API (Render)** | [`https://saudagar-ai-api.onrender.com`](https://saudagar-ai-api.onrender.com) |
| **API Docs (Swagger)** | [`https://saudagar-ai-api.onrender.com/docs`](https://saudagar-ai-api.onrender.com/docs) |

> ⚠️ The backend is hosted on Render's free tier, which **spins down after periods of inactivity**. The first request after idle time may take 30–60 seconds to respond while the instance cold-starts — this is expected, not an outage.

The Flutter frontend is configured to point at this deployed backend by default (see [`frontend/lib/config.dart`](#️-configuration)) rather than `localhost`. Point it at a local backend only when you are actively developing the API.
...
Saudagar AI can be run two ways: point the Flutter frontend at the **already-deployed Render backend** (fastest — no Python setup needed), or run the **full stack locally** for backend development.

### Option A — Frontend only, against the deployed backend (recommended)
...
cd Saudagar_AI/frontend
flutter pub get
flutter run -d chrome --web-port 3000
```

`frontend/lib/config.dart` already points `backendUrl` at `https://saudagar-ai-api.onrender.com`, so no backend setup is required. Open **<http://localhost:3000>** — remember the free-tier Render instance may take 30–60 seconds to wake up on the first request.

### Option B — Full local stack (for backend development)
...
#### 3. Frontend Setup — point it at your local backend

In `frontend/lib/config.dart`, set `backendUrl` to `http://localhost:8001` (instead of the deployed Render URL), then:
...
> Swap in `http://localhost:8001` if you're running the backend locally (Option B above).
...
| -------------------- | ----------------------------------------------- | --------------------------------------------- |
| `backendUrl`        | `https://saudagar-ai-api.onrender.com`      | FastAPI backend URL — deployed on Render. Set to `http://localhost:8001` for local backend development |
